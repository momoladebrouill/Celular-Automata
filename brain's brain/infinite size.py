    ...
    while B:
        ...
        for i,j in lieux:
            if i>-dep.x/SIZE-1 and j>-dep.y/SIZE-1 and i<-dep.x/SIZE+nbs and j<-dep.y/SIZE+nbs: # si on a besoin de le dessiner
                numo=lieux[(i,j)]
                nex[(i,j)]=numo
                if numo==1:pg.draw.rect(f,(255,255,255),((dep.x+i*SIZE,dep.y+j*SIZE),(SIZE,SIZE)))
                elif numo==2:pg.draw.rect(f,(200,200,0),((dep.x+i*SIZE,dep.y+j*SIZE),(SIZE,SIZE)))
                else:pg.draw.rect(f,(255,0,0),((dep.x+i*SIZE,dep.y+j*SIZE),(SIZE,SIZE)))
        lieux.clear()
        lieux.update(nex)
